...
Class Tx_EcDonationrun_Controller_ProjectController Extends Tx_EcDonationrun_Controller_AbstractController {
	...
	Public Function indexAction() {
		 ...
		 *
		 * @param Tx_EcDonationrun_Domain_Model_Project $project The project that is to be displayed.
		 * @return void
		 *
		 */

	Public Function showAction ( Tx_EcDonationrun_Domain_Model_Project $project ) {
		 ...
		 *
		 * @param Tx_EcDonationrun_Domain_Model_Project $project The new project
		 * @return void
		 * @dontvalidate $project
		 *
		 */

	Public Function newAction ( Tx_EcDonationrun_Domain_Model_Project $project=NULL ) {
		$this->view->assign('project' , $project)
	...
	Public Function createAction( Tx_EcDonationrun_Domain_Model_Project $project, $assignments ) {
	...
	Public Function editAction( Tx_EcDonationrun_Domain_Model_Project $project ) {
		$this->view->assign('project' , $project)
		           ->assign('projects', array_merge(Array(NULL), $this->projectRepository->findAll()))
		           ->assign('users'   , $this->userRepository->findAll())
		           ->assign('roles'   , $this->roleRepository->findAll());
	...
	Public Function updateAction( Tx_EcDonationrun_Domain_Model_Project $project, $assignments ) {
		$project->removeAllAssignments();
		ForEach($assignments As $userId => $roleId) {
			If($roleId == 0) Continue;
			$project->assignUser ( $this->userRepository->findByUid((int)$userId),
		 ...
		 *
		 * @param Tx_EcDonationrun_Domain_Model_Project $project The project that is to be deleted
		 * @return void
		 *
		 */

	Public Function deleteAction( Tx_EcDonationrun_Domain_Model_Project $project ) {
