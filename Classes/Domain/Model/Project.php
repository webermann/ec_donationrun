 ...
 *                                                                      */
	 ...
	 * @version    $Id: Project.php 28 2010-09-20 12:26:13Z helmich $
...
Class Tx_EcDonationrun_Domain_Model_Project Extends Tx_Extbase_DomainObject_AbstractEntity {
	...
	Protected $name;
		 ...
		 * @var Tx_EcDonationrun_Domain_Model_Project
		 */
	...
	Protected $tstamp;
		 ...
		 * @var Tx_Extbase_Persistence_ObjectStorage<Tx_EcDonationrun_Domain_Model_Assignment>
		 * @lazy
		 * @cascade remove
		 */
	Protected $assignments;
		 ...
		 * @var Tx_Extbase_Persistence_ObjectStorage<Tx_EcDonationrun_Domain_Model_Project>
		 * @lazy
		 * @cascade remove
		 */
	Protected $children;
	...
	Public Function getName() { Return $this->name; }
		 ...
		 * @return Tx_EcDonationrun_Domain_Model_Project The parent project
		 *
		 */
	...
	Public Function getEditDate() { Return $this->tstamp; }
		 ...
		 * @return Tx_Extbase_Persistence_ObjectStorage<Tx_EcDonationrun_Domain_Model_Assignment>
		 *
		 */
	...
	Public Function getWorkedTime() {
		 ...
		 *
		 * @return Tx_Extbase_Persistence_ObjectStorage<Tx_EcDonationrun_Domain_Model_Timeset>
		 *
		 */
	...
	Public Function getTimesets() {
		 ...
		 * @param Tx_EcDonationrun_Domain_Model_Role $role The role for which the users are to be selected
		 * @return array<Tx_Extbase_Domain_Model_FrontendUser> The users assigned to this project in the specified role
		 *
		 */

	Public Function getUsersForRole(Tx_EcDonationrun_Domain_Model_Role $role) {
		 ...
		 *                                                            is to be retrieved
		 * @return Tx_EcDonationrun_Domain_Model_Assignment       The project assignment for the specified
	...
	Public Function setParent(Tx_EcDonationrun_Domain_Model_Project $parent=NULL) { $this->parent = $parent; }
	...
	Public Function removeAllAssignments() {
		 ...
		 *                                                     to this project.
		 * @param Tx_EcDonationrun_Domain_Model_Role $role The role the user is to be
		 *                                                     assigned in.
		 * @return void
		 *
		 */
	...
	Public Function assignUser ( Tx_Extbase_Domain_Model_FrontendUser   $user,
	                             Tx_EcDonationrun_Domain_Model_Role $role ) {
		If(!$this->getIsAssigned($user)) {
			$assignment = New Tx_EcDonationrun_Domain_Model_Assignment($user, $this, $role);
	...
	Public Function unassignUser(Tx_Extbase_Domain_Model_FrontendUser $user) {
		 ...
		 *
		 * @param Tx_EcDonationrun_Domain_Model_Project $project The new subproject
		 * @return void
		 *
		 */

	Public Function addChild(Tx_EcDonationrun_Domain_Model_Project $project) {
		$project->setParent($this);
		$this->children->attach($project);
	}

	Public Function addAssignment(Tx_EcDonationrun_Domain_Model_Assignment $assignment) {
		$this->assignments->attach($assignment);
	}
